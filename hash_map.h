...
#define HAMP_GET_VALUE(map, key_hash, key_ptr, equality_condition, var_resultant_pointer_pair_ptr){\
  uint32_t _internal_variable_index;\
  HMAP_GET_INDEX(map, key_hash, key_ptr, _internal_variable_index);\
  const struct _pointer_pair *_internal_variable_pair = map[_internal_variable_index];\
  ...
  if(_internal_variable_bool) var_resultant_pointer_pair_ptr = _internal_variable_i; else var_resultant_pointer_pair_ptr = NULL;\
}
